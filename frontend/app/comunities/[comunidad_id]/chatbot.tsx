...
export default function ChatBotScreen() {
  ...
  const authHeaderToken = authToken || globalJwtToken;
  ...
  const fetchDocuments = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/comunities/${normalizedComunidadId}/documents`,
        {
          method: "GET",
      ...
      const message =
        error instanceof Error ? error.message : "Error al listar documentos";
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/comunities/${normalizedComunidadId}/chatbot`,
        {
          method: "POST",
          headers: {
  ...
  const handleUploadDocument = async () => {
    if (!selectedFile && (!docTitle.trim() || !docContent.trim())) {
      Alert.alert(
        "Atención",
        "Escribe el contenido o selecciona un archivo PDF/TXT.",
      );
      return;
    }
    ...
    try {
      ...
      if (selectedFile) {
        ...
        if (Platform.OS === "web") {
          const webAsset =
            selectedFile as DocumentPicker.DocumentPickerAsset & {
              file?: File;
            };
          const fileToUpload = webAsset.file;
          if (!fileToUpload)
            throw new Error("No se pudo obtener el archivo del selector.");
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : "Error al borrar documento";
      Alert.alert("Error", message);
    } finally {
      setIsDeleting(false);
    }
      ...
      >
        ...
        {item.source && (
          <View style={styles.sourceTag}>
            <Text style={[styles.sourceText, { fontWeight: "700" }]}>
              Fuente: {item.source.type}
            </Text>
      ...
      >
        <View style={{ flex: 1, flexDirection: isDesktop ? "row" : "column" }}>
          ...
          {isManager && (isDesktop || activeTab === "docs") && (
            <View
              style={[styles.docsPanel, { width: isDesktop ? 380 : "100%" }]}
            >
              <ScrollView
              ...
              >
                ...
                {!selectedFile && (
                  <>
                    ...
                    <TextInput
                      style={[
                        styles.docInput,
                        { flex: 1, textAlignVertical: "top", minHeight: 120 },
                      ]}
                ...
                <TouchableOpacity
                  ...
                  style={[
                    styles.uploadBtn,
                    { backgroundColor: isUploading ? "#CBD5E1" : "#4F46E5" },
                  ]}
                >
                  {isUploading ? (
                ...
                {docsError && (
                  <Text style={styles.docsErrorText}>{docsError}</Text>
                )}
...
const TabItem = ({ label, icon, active, onPress }: TabItemProps) => (
  ...
  >
    {icon}
    <Text style={[styles.tabLabel, { color: active ? "#4F46E5" : "#64748B" }]}>
      {label}
    </Text>
  </TouchableOpacity>
);
