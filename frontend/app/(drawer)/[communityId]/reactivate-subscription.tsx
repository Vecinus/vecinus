...
export default function ReactivateSubscriptionScreen() {
  ...
  const handleSubmitForm = async () => {
    if (!communityId) {
      showError('Comunidad no disponible', 'No se pudo resolver la comunidad que quieres reactivar.');
      return;
    }
    ...
    try {
      ...
      const created = await paymentsApi.createSubscriptionReactivationOrder(communityId, {
        plan,
      ...
      showError(
        ...
        getErrorMessage(error, 'Error de red al crear la orden de reactivación.'),
      );
    } finally {
  ...
  const renderFormStep = () => (
    <View>
      <View className="mb-8 mt-4 items-center">
        ...
        <Text className="text-center text-2xl font-bold text-foreground">Reactivar suscripción</Text>
        <Text className="mt-2 text-center text-sm text-muted-foreground">
          Esta comunidad ya estuvo suscrita. Vas a elegir de nuevo el plan y firmar un mandato nuevo.
        </Text>
      </View>
      ...
      <View className="gap-4">
        <View className="gap-2">
          ...
          <Text className="text-xs text-muted-foreground ml-1">
            Este valor se usará para recalcular la cuota mensual y actualizar la configuración de la comunidad.
          </Text>
          {householdCountError ? (
  ...
  const renderPayingStep = () => (
    <View>
      <View className="mb-6 mt-4 items-center">
        ...
        <Text className="mt-2 text-center text-sm text-muted-foreground">
          Revisa los datos y abre la pasarela de pago para autorizar el nuevo mandato.
        </Text>
      </View>
      ...
      <View className="mt-8 gap-3">
        <Button onPress={handleOpenGateway} className="h-14 rounded-xl">
          <Text className="text-lg font-bold text-primary-foreground">Firmar mandato</Text>
        </Button>
