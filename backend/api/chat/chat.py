...
def create_group_channel(
    ...
    members_res = (
        ...
        .select("profile_id")
        .eq("association_id", str(channel_in.association_id))
        .execute()
    )
...
def update_group_channel(
    ...
    if channel_data.get("is_direct_message"):
        raise HTTPException(
            status_code=400,
            detail="Cannot edit direct messages through this endpoint",
        )
...
def delete_group_channel(
    ...
    if channel_data.get("is_direct_message"):
        raise HTTPException(
            status_code=400,
            detail="Cannot delete direct messages through this endpoint",
        )
...
def get_channel_messages(
    ...
    messages_res = (
        ...
        .select("*, sender:sender_id(id, username, avatar_url, created_at)")
...
def create_direct_message(
    ...
    target_access_res = (
        ...
        .select("*")
        .eq("channel_id", str(channel_id))
        .eq("user_id", str(dm_in.target_user_id))
        .execute()
    )
    if not target_access_res.data:
        raise HTTPException(
            status_code=400,
            detail="Target user is not a participant of this channel",
        )
...
async def update_message(
    ...
    supabase.rpc(
        ...
        {"p_reference_id": str(message_id), "p_content": msg_preview},
    ).execute()

    # 4. Retransmitir evento de edición al WebSocket
    broadcast_data = {"event": "message_edited", "message": updated_message}
