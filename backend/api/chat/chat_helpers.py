...
def verify_message_ownership(
    ...
    if str(original_msg["sender_id"]) != str(user_id):
        raise HTTPException(
            status_code=403,
            detail="Not authorized to perform this action on this message",
        )
...
def verify_association_president(association_id: UUID | str, user_id: str, supabase: Client):
    ...
    if str(user_role) != "4":
        raise HTTPException(
            status_code=403,
            detail="Association president access required for this action",
        )

    return membership_res.data[0]


def verify_association_admin_or_presidente(association_id: UUID | str, user_id: str, supabase: Client):
    """Verifica que un usuario tiene rol de administrador (role=1)
    o presidente (role=4) en la comunidad dada. Lanza 403 o 404."""
    membership_res = (
        supabase.table("memberships")
        .select("role")
        .eq("association_id", str(association_id))
    ...
    user_role = str(membership_res.data[0].get("role"))

    if user_role not in ("1", "4"):
        raise HTTPException(
            status_code=403,
            detail="Admin or Association president access required for this action",
        )
