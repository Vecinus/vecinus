...
def create_direct_message(
    ...
    target_access_res = supabase.table("channel_participants").select("*").eq("channel_id", str(channel_id)).eq("user_id", str(dm_in.target_user_id)).execute()
    ...
    dm_channels_res = supabase.table("chat_channels").select("id, is_blocked").eq("association_id", association_id).eq("is_direct_message", True).execute()
    if dm_channels_res.data:
        dm_channel_ids = [c["id"] for c in dm_channels_res.data]
        
        # Buscar en qué canales de DM participo yo
        my_dms_res = supabase.table("channel_participants").select("channel_id").eq("user_id", my_user_id).in_("channel_id", dm_channel_ids).execute()
        my_dm_ids = [str(p["channel_id"]) for p in my_dms_res.data]
        
        if my_dm_ids:
            # Buscar en cuáles de esos participa el target
            target_dms_res = supabase.table("channel_participants").select("channel_id").eq("user_id", target_user_id).in_("channel_id", my_dm_ids).execute()
...
def block_direct_message_channel(
    ...
    update_res = supabase.table("chat_channels").update({
        "is_blocked": True,
...
async def send_message(
    ...
    insert_res = supabase.table("messages").insert(new_msg).execute()
    ...
    all_participants_res = supabase.table("channel_participants").select("user_id").eq("channel_id", str(channel_id)).execute()
