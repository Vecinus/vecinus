...
async def upload_document(
    ...
    if "application/json" in content_type:
        ...
        if not title or not content:
            raise HTTPException(status_code=400, detail="Faltan campos 'title' o 'content' en el JSON.")
            
        chunks = index_document(comunidad_id, title, content)
        return {"message": f"Documento '{title}' indexado con éxito", "chunks": chunks}
    ...
    # CASO 2: Si es un Archivo Físico (PDF o TXT)
        ...
        if not texto_extraido.strip():
            raise HTTPException(status_code=400, detail="El documento está vacío o no se pudo extraer el texto.")

        chunks = index_document(comunidad_id, file.filename, texto_extraido)
        return {"message": f"Documento '{file.filename}' indexado con éxito", "chunks": chunks}

    else:
        raise HTTPException(status_code=400, detail="Content-Type no soportado. Usa JSON o Form-Data.")