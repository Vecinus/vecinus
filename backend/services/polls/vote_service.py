...
class VoteService:
    ...
    def cast_vote(self, poll_id: UUID, vote_data: VoteCreate):
        ...
        token_res = (
            self.supabase.table("dev_s2.voting_tokens")
            .select("*")
        ...
        poll_res = self.supabase.table("dev_s2.polls").select("options").eq("id", str(poll_id)).execute()
        ...
        member_res = self.supabase.table("dev_s2.memberships").select("property_id").eq("id", membership_id).execute()
        property_id = member_res.data[0]["property_id"]

        prop_res = (
            self.supabase.table("dev_s2.properties").select("coefficient, is_defaulter").eq("id", property_id).execute()
        )
        ...
        try:
            vote_res = self.supabase.table("dev_s2.votes").insert(vote_insert_data).execute()
        except Exception as e:
            raise HTTPException(
                status_code=400, detail=f"Ya se ha registrado un voto para esta propiedad, error: {str(e)}"
            )

        self.supabase.table("dev_s2.voting_tokens").update({"is_used": True}).eq(
