...
def test_variables_entorno_cargadas():
    """Verifica que las variables críticas están configuradas antes de lanzar tests"""
...
def test_chatbot_pregunta_irrelevante_devuelve_fallback():
    """PRUEBA NEGATIVA 2: Pregunta fuera de contexto para probar el anti-alucinaciones"""
    comunidad_id = 1
    payload = {
    ...
    # Debe saltar el threshold de Pinecone o la instrucción restrictiva de Gemini
    assert "no he encontrado" in respuesta or "no aparece en los estatutos" in respuesta or "no puedo encontrar" in respuesta
...
def test_chatbot_documento_ya_existente():
    """PRUEBA POSITIVA 1: Preguntar sobre un documento que ya sabemos que está subido (prueba.txt)"""
    comunidad_id = 1
    payload = {
        "comunidad_id": str(comunidad_id),
...
def test_chatbot_flujo_completo_con_subida_documento():
    """PRUEBA POSITIVA 2: Subir archivo -> Esperar a Pinecone -> Preguntar"""
    ...
    # ⚠️ Pinecone necesita unos segundos (Eventual Consistency) para que el vector sea buscable
    print("[2/3] Documento subido. Esperando 15 segundos a que Pinecone lo indexe...")
    time.sleep(15) 
